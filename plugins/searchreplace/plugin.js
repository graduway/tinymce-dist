...
var searchreplace = (function () {
  ...
  var $_972u6fk1jl265gy8 = { findAndReplaceDOMText: findAndReplaceDOMText };
  ...
  var markAllMatches = function (editor, currentIndexState, regex) {
    ...
    return $_972u6fk1jl265gy8.findAndReplaceDOMText(regex, node, marker, false, editor.schema);
  };
  ...
  var get = function (editor, currentIndexState) {
    var done = function (keepEditorSelection) {
      return $_34htgxjzjl265gy3.done(editor, currentIndexState, keepEditorSelection);
    };
    var find = function (text, matchCase, wholeWord) {
      return $_34htgxjzjl265gy3.find(editor, currentIndexState, text, matchCase, wholeWord);
    };
    var next = function () {
      return $_34htgxjzjl265gy3.next(editor, currentIndexState);
    };
    var prev = function () {
      return $_34htgxjzjl265gy3.prev(editor, currentIndexState);
    };
    var replace = function (text, forward, all) {
      return $_34htgxjzjl265gy3.replace(editor, currentIndexState, text, forward, all);
    };
    return {
      done: done,
      find: find,
  ...
  var open = function (editor, currentIndexState) {
    ...
    function updateButtonStates() {
      win.statusbar.find('#next').disabled($_34htgxjzjl265gy3.hasNext(editor, currentIndexState) === false);
      win.statusbar.find('#prev').disabled($_34htgxjzjl265gy3.hasPrev(editor, currentIndexState) === false);
    }
    ...
    var win = editor.windowManager.open({
      ...
      onClose: function () {
        editor.focus();
        $_34htgxjzjl265gy3.done(editor, currentIndexState);
        editor.undoManager.add();
      },
      onSubmit: function (e) {
        ...
        if (!text.length) {
          $_34htgxjzjl265gy3.done(editor, currentIndexState, false);
          win.statusbar.items().slice(1).disabled(true);
          return;
        }
        if (last.text === text && last.caseState === caseState && last.wholeWord === wholeWord) {
          if (!$_34htgxjzjl265gy3.hasNext(editor, currentIndexState)) {
            notFoundAlert();
            return;
          }
          $_34htgxjzjl265gy3.next(editor, currentIndexState);
          updateButtonStates();
          return;
        }
        count = $_34htgxjzjl265gy3.find(editor, currentIndexState, text, caseState, wholeWord);
        if (!count) {
          notFoundAlert();
        }
      ...
      buttons: [
        ...
        },
        {
          ...
          disabled: true,
          onclick: function () {
            if (!$_34htgxjzjl265gy3.replace(editor, currentIndexState, win.find('#replace').value())) {
              win.statusbar.items().slice(1).disabled(true);
              currentIndexState.set(-1);
              last = {};
            }
        ...
        },
        {
          ...
          disabled: true,
          onclick: function () {
            $_34htgxjzjl265gy3.replace(editor, currentIndexState, win.find('#replace').value(), true, true);
            win.statusbar.items().slice(1).disabled(true);
            last = {};
          }
        },
        ...
        },
        {
          ...
          disabled: true,
          onclick: function () {
            $_34htgxjzjl265gy3.prev(editor, currentIndexState);
            updateButtonStates();
          }
        },
        {
  ...
  var $_dhvwqnk3jl265gye = { open: open };

  var register = function (editor, currentIndexState) {
    editor.addCommand('SearchReplace', function () {
      $_dhvwqnk3jl265gye.open(editor, currentIndexState);
    });
  };
  var $_g442rnk2jl265gyc = { register: register };
  ...
  var $_49pz06k4jl265gyi = { register: register$1 };

  global.add('searchreplace', function (editor) {
    var currentIndexState = Cell(-1);
    $_g442rnk2jl265gyc.register(editor, currentIndexState);
    $_49pz06k4jl265gyi.register(editor, currentIndexState);
    return $_5w0c5mjyjl265gy0.get(editor, currentIndexState);
  });
  function Plugin () {
  }

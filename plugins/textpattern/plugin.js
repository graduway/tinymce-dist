...
var textpattern = (function () {
  ...
  var $_dylb9crtjl265k3n = { getPatterns: getPatterns };
  ...
  var patternFromRng = function (patterns, rng, space) {
    ...
    var endPattern = $_fxpxj6s1jl265k41.findEndPattern(patterns, text, rng.startOffset, delta);
    ...
    var startPattern = $_fxpxj6s1jl265k41.findPattern(patterns, patternRng.toString());
  ...
  var applyBlockFormat = function (editor, patterns) {
    ...
    if (textBlockElm) {
      ...
      if (firstTextNode) {
        pattern = $_fxpxj6s1jl265k41.findPattern(patterns, firstTextNode.data);
        if (!pattern) {
          return;
        }
  ...
  function handleEnter(editor, patterns) {
    ...
    wrappedTextNode = $_c8zvhlryjl265k3v.applyInlineFormatEnter(editor, patterns);
    if (wrappedTextNode) {
    ...
    $_c8zvhlryjl265k3v.applyBlockFormat(editor, patterns);
  }
  function handleInlineKey(editor, patterns) {
    var wrappedTextNode, lastChar, lastCharNode, rng, dom;
    wrappedTextNode = $_c8zvhlryjl265k3v.applyInlineFormatSpace(editor, patterns);
    if (wrappedTextNode) {
      dom = editor.dom;
  ...
  var setup = function (editor, patternsState) {
    ...
    editor.on('keydown', function (e) {
      if (e.keyCode === 13 && !global$2.modifierPressed(e)) {
        $_42nu0rrxjl265k3r.handleEnter(editor, patternsState.get());
      }
    }, true);
    editor.on('keyup', function (e) {
      if ($_42nu0rrxjl265k3r.checkKeyCode(keyCodes, e)) {
        $_42nu0rrxjl265k3r.handleInlineKey(editor, patternsState.get());
      }
    });
    editor.on('keypress', function (e) {
      if ($_42nu0rrxjl265k3r.checkCharCode(charCodes, e)) {
  ...
  var $_3sn7q6rujl265k3o = { setup: setup };

  global.add('textpattern', function (editor) {
    var patternsState = Cell($_dylb9crtjl265k3n.getPatterns(editor.settings));
    $_3sn7q6rujl265k3o.setup(editor, patternsState);
    return $_7snc3ursjl265k3l.get(patternsState);
  });
  function Plugin () {
  }
